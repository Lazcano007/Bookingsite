...
const resend = useResend ? new Resend(process.env.RESEND_API_KEY as string) : null;

const sender = !useResend ? nodemailer.createTransport({
    service: "gmail",
    auth: {
...
export async function sendBookingEmail (to: string, booking: any) {
    ...
    const from = process.env.EMAIL_FROM || (process.env.EMAIL_USER ? `Fresh Line Barber <${process.env.EMAIL_USER}>` : 'Fresh Line Barber <no-reply@example.com>');
